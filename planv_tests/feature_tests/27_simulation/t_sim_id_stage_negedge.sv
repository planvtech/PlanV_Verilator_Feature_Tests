...
module t_sim_id_stage_negedge();
  ...
  always @(posedge clk) begin
    if (!rst_n) begin
      if (uut.issue_q[0].valid != 0) begin
        `DBG(("ERROR: issue_q[0].valid should be 0 after reset"))
      end
    end
    if (fetch_entry_valid[0] && !uut.issue_q[0].valid) begin
      if (uut.issue_n[0].valid != 1) begin
        `DBG(("ERROR: issue_n[0].valid should be 1 when fetch_entry_valid[0] is asserted and issue_q[0] is not valid"))
      end
      if (uut.issue_n[0].sbe != fetch_entry[0]) begin
        `DBG(("ERROR: issue_n[0].sbe should match fetch_entry[0]"))
      end
    end
    if (issue_instr_ack[0]) begin
      if (uut.issue_n[0].valid != 0) begin
        `DBG(("ERROR: issue_n[0].valid should be 0 when issue_instr_ack[0] is asserted"))
      end
    end
    if (flush) begin
      if (uut.issue_n[0].valid != 0) begin
        `DBG(("ERROR: issue_n[0].valid should be 0 when flush is asserted"))
      end
    end
  end
  ...
  // Dump waveforms
  initial begin
    $dumpfile("t_sim_id_stage_negedge.vcd");
    $dumpvars(0, t_sim_id_stage_negedge);
  end
