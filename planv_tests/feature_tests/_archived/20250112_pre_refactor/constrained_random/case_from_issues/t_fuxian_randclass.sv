// Test if rand class-type members cause randomize() bug
// UVM test has: rand uvme_fifo_cfg_c env_cfg;
/* verilator lint_off WIDTHTRUNC */

// Config class (like uvme_fifo_cfg_c)
class cfg_c;
   rand bit [7:0] cfg_value;

   constraint cfg_con {
      cfg_value > 5;
   }

   function new();
   endfunction
endclass

// Test class with rand class-type member (like UVM test)
class my_test_c;
   rand cfg_c my_cfg;  // KEY: rand class-type member
   rand bit [7:0] value;
   int success;

   constraint value_con {
      value > 10;
      value < 20;
   }

   function new();
   endfunction

   // Method that calls randomize
   function void do_randomize();
      success = this.randomize();
      $display("Randomize: success=%0d, value=%0d, cfg_value=%0d",
               success, value, my_cfg.cfg_value);
   endfunction
endclass


module t_fuxian_randclass;

   initial begin
      my_test_c test;

      $display("\n========================================");
      $display("Test: rand Class Member Impact");
      $display("========================================\n");

      test = new();
      test.my_cfg = new();
      test.do_randomize();

      $display("\n========================================");
      if (test.value > 10 && test.value < 20) begin
         $display("*** PASS *** value=%0d (range 11-19)", test.value);
      end else begin
         $display("*** FAIL *** value=%0d (should be 11-19)", test.value);
         $display("Bug: rand class member causes wrong randomize()");
      end
      $display("========================================\n");

      $finish;
   end

endmodule
/* verilator lint_on WIDTHTRUNC */
