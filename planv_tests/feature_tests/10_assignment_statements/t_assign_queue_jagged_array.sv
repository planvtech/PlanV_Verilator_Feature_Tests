...
//
// This test validates: aggregate assignment to jagged arrays (queue of queues)

`include "test_utils.svh"

module t_assign_queue_jagged_array;
    typedef int T_QI[$];
    ...
    task automatic print_and_check();
        ...
        for (i = 0; i < jagged_array.size(); i++) begin
            for (j = 0; j < jagged_array[i].size(); j++) begin
                `DBG(("jagged_array[%0d][%0d] = %0d", i, j, jagged_array[i][j]))
