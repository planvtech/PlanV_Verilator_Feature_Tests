...
class AssocArrTest;
    ...
    function void check_and_display();
        ...
        foreach (int_index_arr[i]) begin
            ...
            if (!(int_index_arr[i] inside {10, 20, 30, 40})) begin
                $error("Constraint violated for int_index_arr[%0d]: %0d", i, int_index_arr[i]);
            end
        end
        ...
        foreach (string_index_arr[name]) begin
            ...
            if ((name == "Alice" && string_index_arr[name] != 35) ||
                ...
                (name == "Charlie" && string_index_arr[name] <= 25)) begin
                $error("Constraint violated for string_index_arr[%s]: %0d", name, string_index_arr[name]);
            end
        end
    endfunction
endclass
