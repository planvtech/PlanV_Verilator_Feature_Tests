...
//
// This test validates: simulation behavior for ID stage negative edge
...
module t_sim_id_stage_negedge();
  ...
  always @(posedge clk) begin
    if (!rst_n) begin
      if (uut.issue_q[0].valid != 0) `DBG(("ERROR: issue_q[0].valid should be 0 after reset"))
      else `DBG(("PASS: issue_q[0].valid is 0 after reset"))
    end
    if (fetch_entry_valid[0] && !uut.issue_q[0].valid) begin
      if (uut.issue_n[0].valid != 1) `DBG(("ERROR: issue_n[0].valid should be 1 when fetch_entry_valid[0] is asserted and issue_q[0] is not valid"))
      else `DBG(("PASS: issue_n[0].valid is 1 when fetch_entry_valid[0] is asserted and issue_q[0] is not valid"))
      
      if (uut.issue_n[0].sbe != fetch_entry[0]) `DBG(("ERROR: issue_n[0].sbe should match fetch_entry[0]"))
      else `DBG(("PASS: issue_n[0].sbe matches fetch_entry[0]"))
    end
    if (issue_instr_ack[0]) begin
      if (uut.issue_n[0].valid != 0) `DBG(("ERROR: issue_n[0].valid should be 0 when issue_instr_ack[0] is asserted"))
      else `DBG(("PASS: issue_n[0].valid is 0 when issue_instr_ack[0] is asserted"))
    end
    if (flush) begin
      if (uut.issue_n[0].valid != 0) `DBG(("ERROR: issue_n[0].valid should be 0 when flush is asserted"))
      else `DBG(("PASS: issue_n[0].valid is 0 when flush is asserted"))
    end
  end
