...
module tb_simple_issue_test;
  ...
  initial begin
    ...
    if (issue_q[0].valid !== issue_n[0].valid || issue_q[0].sbe !== decoded_instruction[0]) begin
      $display("Error: issue_q[0] does not follow issue_n[0]");
      $stop;
    end
    ...
    if (issue_q[1].valid !== issue_n[1].valid || issue_q[1].sbe !== decoded_instruction[1]) begin
      $display("Error: issue_q[1] does not follow issue_n[1]");
      $stop;
    end
    ...
    if (issue_q[0].valid !== 0 || issue_q[1].valid !== 0) begin
      $display("Error: issue_q not cleared after flush");
      $stop;
    end
    ...
    if (issue_q[0].sbe !== 32'hABCDEF01 || issue_q[0].orig_instr !== 32'h10203040) begin
      $display("Error: issue_q[0] not updated correctly on repeated fetch");
      $stop;
    end
