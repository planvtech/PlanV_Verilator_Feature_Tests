...
//
// This test validates: unique constraint with variable naming
...
class SimilarNames;
  ...
  function bit check();
    ...
    if (pass) `DBG(("SimilarNames: PASSED"))
    return pass;
  endfunction
endclass
...
class MultipleRandomize;
  ...
  function bit check(int iteration);
    ...
    `DBG(("Iteration %0d: PASSED - values: %p", iteration, values))
    return 1;
  endfunction
endclass
...
module t_constraint_unique_naming;
  initial begin
    ...
    `DBG(("=== Test 1: Similar Array Names ==="))
    sn = new();
