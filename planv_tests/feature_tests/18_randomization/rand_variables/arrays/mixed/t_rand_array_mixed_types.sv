...
//
// This test validates: rand mixed array types (packed, unpacked, dynamic)
...
module t_rand_array_mixed_types;
  ...
  initial begin
    ...
    repeat(2) begin
      ...
      `DBG(("Randomized Packed Array:"))
      foreach (packed_class.packed_array[i, j, k]) begin
        `DBG(("packed_array[%0d][%0d][%0d] = %0h", i, j, k, packed_class.packed_array[i][j][k]))
      end
    end
    ...
    repeat(2) begin
      ...
      `DBG(("Randomized Unpacked Array:"))
      foreach (unpacked_class.unpacked_array[i, j]) begin
        `DBG(("unpacked_array[%0d][%0d] = %0h", i, j, unpacked_class.unpacked_array[i][j]))
      end
    end
    ...
    repeat(2) begin
      ...
      `DBG(("Randomized Dynamic 1D Array:"))
      foreach (dynamic_class.dynamic_array_1d[i]) begin
        `DBG(("dynamic_array_1d[%0d] = %0d", i, dynamic_class.dynamic_array_1d[i]))
      end

      // Display the randomized dynamic 2D array
      `DBG(("Randomized Dynamic 2D Array:"))
      foreach (dynamic_class.dynamic_array_2d[i, j]) begin
        `DBG(("dynamic_array_2d[%0d][%0d] = %0d", i, j, dynamic_class.dynamic_array_2d[i][j]))
      end
    end
    ...
    repeat(2) begin
      ...
      `DBG(("Randomized Struct Array:"))
      foreach (struct_with_array_class.struct_with_array.byte_array[i]) begin
        `DBG(("struct_with_array.byte_array[%0d] = %0h", i, struct_with_array_class.struct_with_array.byte_array[i]))
      end
    end
    ...
    repeat(2) begin
      ...
      `DBG(("Randomized Associative Array 1D:"))
      `DBG(("associative_array_1d[\"key1\"] = %0d", associative_array_class.associative_array_1d["key1"]))
      `DBG(("associative_array_1d[\"key2\"] = %0d", associative_array_class.associative_array_1d["key2"]))

      // Display the randomized associative array 3D
      `DBG(("Randomized Associative Array 3D:"))
      foreach(associative_array_class.associative_array_3d[key1, key2, key3]) begin
        `DBG(("associative_array_3d[\"%s\"][\"%s\"][\"%s\"] = %0d", key1, key2, key3, associative_array_class.associative_array_3d[key1][key2][key3]))
      end
    end
    ...
    repeat(2) begin
      ...
      `DBG(("Randomized Queue Array 1D:"))
      foreach (queue_class.queue_array_1d[i]) begin
        `DBG(("queue_array_1d[%0d] = %0d", i, queue_class.queue_array_1d[i]))
      end

      // Display the randomized queue array 2D
      `DBG(("Randomized Queue Array 2D:"))
      foreach (queue_class.queue_array_2d[i, j]) begin
        `DBG(("queue_array_2d[%0d][%0d] = %0d", i, j, queue_class.queue_array_2d[i][j]))
      end
    end
