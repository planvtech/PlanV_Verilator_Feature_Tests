...
//
// This test validates: constraints on 1D mixed array types (variant 2)
  ...
  function void check();
    foreach (dynamic_array[i]) begin
      if (dynamic_array[i] < 1 || dynamic_array[i] > 10) begin
        `DBG(("Error: dynamic_array[%0d] = %0d, out of range [1:10]", i, dynamic_array[i]))
        $stop;
      end
    end

    if (associative_array["key1"] < 100 || associative_array["key1"] > 200) begin
      `DBG(("Error: associative_array['key1'] = %0d, out of range [100:200]", associative_array["key1"]))
      $stop;
    end
    if (associative_array["key2"] < 300 || associative_array["key2"] > 400) begin
      `DBG(("Error: associative_array['key2'] = %0d, out of range [300:400]", associative_array["key2"]))
      $stop;
    end

    foreach (fixed_size_array[i]) begin
      if (fixed_size_array[i] != i) begin
        `DBG(("Error: fixed_size_array[%0d] = %0h, expected %0d", i, fixed_size_array[i], i))
        $stop;
      end
    end
    ...
    foreach (queue[i]) begin
      if (queue[i] < 50 || queue[i] > 100) begin
        `DBG(("Error: queue[%0d] = %0d, out of range [50:100]", i, queue[i]))
        $stop;
      end
    end
  endfunction
...
module t_rand_array_mixed_1d_case2;
  ...
  initial begin
    ...
    if (!my_array.randomize()) begin
      `DBG(("Constrained mixed array randomization failed."))
      $stop;
    end
    ...
    `DBG(("Randomization successful!"))

    `DBG(("Dynamic array values:"))
    foreach (my_array.dynamic_array[i]) begin
      `DBG(("dynamic_array[%0d] = %0d", i, my_array.dynamic_array[i]))
    end

    `DBG(("Associative array values:"))
    `DBG(("associative_array['key1'] = %0d", my_array.associative_array["key1"]))
    `DBG(("associative_array['key2'] = %0d", my_array.associative_array["key2"]))

    `DBG(("Fixed-size array values:"))
    foreach (my_array.fixed_size_array[i]) begin
      `DBG(("fixed_size_array[%0d] = %0h", i, my_array.fixed_size_array[i]))
    end

    `DBG(("Queue array values:"))
    foreach (my_array.queue[i]) begin
      `DBG(("queue[%0d] = %0d", i, my_array.queue[i]))
    end
