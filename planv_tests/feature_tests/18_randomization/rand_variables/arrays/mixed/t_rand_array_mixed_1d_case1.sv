...
//
// This test validates: constraints on 1D mixed array types (variant 1)
...
class constrained_mixed_array;
  ...
  function void check();
    `DBG(("Checking arrays for constraint compliance:"))

    // Check packed array
    if (packed_array != 8'hAA) begin
      `DBG(("Error: packed_array = %0h, expected = 8'hAA", packed_array))
      $stop;
    end else begin
      `DBG(("Packed array meets constraint: packed_array = %0h", packed_array))
    end

    // Check unpacked array
    if (unpacked_array[0] != 1'b1 || unpacked_array[1] != 1'b1 || unpacked_array[2] != 1'b0) begin
      `DBG(("Error: unpacked_array does not match constraint."))
      $stop;
    end else begin
      for (int i = 0; i < 4; i++) begin
        `DBG(("unpacked_array[%0d] = %0b", i, unpacked_array[i]))
      end
    end
    ...
    for (int i = 0; i < dynamic_array.size(); i++) begin
      if (dynamic_array[i] != i + 1) begin
        `DBG(("Error: dynamic_array[%0d] = %0d, expected = %0d", i, dynamic_array[i], i + 1))
        $stop;
      end
    end
    `DBG(("Dynamic array meets constraint: %p", dynamic_array))

    // Check associative array
    if (associative_array["key1"] != 100 || associative_array["key2"] != 200) begin
      `DBG(("Error: associative_array does not match constraint."))
      $stop;
    end else begin
      `DBG(("associative_array['key1'] = %0d", associative_array["key1"]))
      `DBG(("associative_array['key2'] = %0d", associative_array["key2"]))
    end

    // Check fixed size array
    for (int i = 0; i < 4; i++) begin
      if (fixed_size_array[i] != i + 1) begin
        `DBG(("Error: fixed_size_array[%0d] = %0h, expected = %0h", i, fixed_size_array[i], i + 1))
        $stop;
      end
    end
    `DBG(("Fixed size array meets constraint: %p", fixed_size_array))

    // Check queue
    if (queue.size() != 2 || queue[0] < 50 || queue[0] > 60 || queue[1] < 70 || queue[1] > 80) begin
      `DBG(("Error: queue does not match constraint."))
      $stop;
    end else begin
      for (int i = 0; i < queue.size(); i++) begin
        `DBG(("queue[%0d] = %0d", i, queue[i]))
      end
    end
  endfunction
...
module t_rand_array_mixed_1d_case1;
