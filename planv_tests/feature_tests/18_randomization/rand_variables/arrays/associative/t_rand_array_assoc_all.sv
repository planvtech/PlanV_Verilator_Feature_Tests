...
//
// This test validates: constraints on all associative array features
...
module t_rand_array_assoc_all;
    ...
    initial begin
        ...
        if (!success) $stop;
        `DBG(("[AssocArrayEnum] Randomization successful."))
        foreach (enum_arr.colors[i]) begin
            `DBG(("  colors[%s] = %0d", i.name(), enum_arr.colors[i]))
        end

        success = packed_arr.randomize();
        if (!success) $stop;
        `DBG(("[AssocArrayPackedStruct] Randomization successful."))
        foreach (packed_arr.data[i]) begin
            `DBG(("  data[high: %0d, low: %0d] = %0d", i.high, i.low, packed_arr.data[i]))
        end

        success = unpacked_arr.randomize();
        if (!success) $stop;
        `DBG(("[AssocArrayUnpackedStruct] Randomization successful."))
        foreach (unpacked_arr.data[i]) begin
            `DBG(("  data[a: %0d, b: %0d] = %0d", i.a, i.b, unpacked_arr.data[i]))
        end

        success = array_index_arr.randomize();
        if (!success) $stop;
        `DBG(("[AssocArrayArrayIndex] Randomization successful."))
        foreach (array_index_arr.data[i]) begin
            `DBG(("  data[[%0d, %0d]] = %0d", i[0], i[1], array_index_arr.data[i]))
        end

        success = integral_arr.randomize();
        if (!success) $stop;
        `DBG(("[AssocArrayIntegral] Randomization successful."))
        foreach (integral_arr.int_index[i]) begin
            `DBG(("  int_index[%0d] = %0d", i, integral_arr.int_index[i]))
        end
        foreach (integral_arr.str_index[i]) begin
            `DBG(("  str_index[\"%s\"] = %0d", i, integral_arr.str_index[i]))
        end

        success = class_index_arr.randomize();
        if (!success) $stop;
        `DBG(("[AssocArrayClass] Randomization successful."))
        foreach (class_index_arr.data[i]) begin
            `DBG(("  data[class_id: %0d] = %0d", i.id, class_index_arr.data[i]))
        end
