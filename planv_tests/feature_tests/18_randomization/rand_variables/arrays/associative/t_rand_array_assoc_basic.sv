...
//
// This test validates: basic associative array constraints
...
class constrained_1d_associative_array;
  ...
  function void debug_display();
    `DBG(("string_index_arr[\"key1\"] = %0d", string_index_arr["key1"]))
    `DBG(("string_index_arr[\"key2\"] = %0d", string_index_arr["key2"]))
    `DBG(("int_index_arr[40000] = %0d", int_index_arr[40000]))
    `DBG(("int_index_arr[2000000000] = %0d", int_index_arr[2000000000]))
    `DBG(("shortint_index_arr[2000] = %0d", shortint_index_arr[2000]))
    `DBG(("longint_index_arr[4000000000] = %0d", longint_index_arr[64'd4000000000]))
    `DBG(("byte_index_arr[255] = %0d", byte_index_arr[8'd255]))
    `DBG(("bit_index_arr[30] = %0d", bit_index_arr[6'd30]))
    `DBG(("bit_index_arr_1[66] = %0d", bit_index_arr_1[56'd66]))
    `DBG(("logic_index_arr[3] = %0d", logic_index_arr[4'd3]))
  endfunction
...
class constrained_2d_associative_array;
  ...
  function void debug_display();
    `DBG(("string_int_index_arr[\"key1\"][2000] = %0d", string_int_index_arr["key1"][2000]))
    `DBG(("string_int_index_arr[\"key2\"][3000] = %0d", string_int_index_arr["key2"][3000]))
    `DBG(("int_bit_index_arr[40000][30] = %0d", int_bit_index_arr[40000][6'd30]))
    `DBG(("int_bit_index_arr[50000][40] = %0d", int_bit_index_arr[50000][6'd40]))
    `DBG(("string_bit_index_arr[\"key3\"][100] = %0d", string_bit_index_arr["key3"][8'd100]))
    `DBG(("string_bit_index_arr[\"key4\"][200] = %0d", string_bit_index_arr["key4"][8'd200]))
    `DBG(("unpacked_assoc_array_2d[\"key5\"][0] = %0d", unpacked_assoc_array_2d["key5"][0]))
  endfunction

endclass

module t_rand_array_assoc_basic;
  ...
  initial begin
    ...
    if (!my_2d_array.randomize()) begin
      `DBG(("Constrained 2D associative array randomization failed."))
      $stop;
    end
