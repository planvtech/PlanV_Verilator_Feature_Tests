...
//
// This test validates: constrained randomization of basic data types
...
class BasicRandTest;
    ...
    function void check();
        if (!(enum_value inside {ENUM_ONE, ENUM_TWO, ENUM_THREE})) begin
            `DBG(("Error: enum_value = %0d is out of bounds", enum_value))
            $stop;
        end
        if (!((random_32_bit & 32'hFFFF0000) == 32'hABCD0000)) begin
            `DBG(("Error: random_32_bit = %h does not satisfy the constraint", random_32_bit))
            $stop;
        end
        if (single_bit != 1) begin
            `DBG(("Error: single_bit = %b does not equal 1", single_bit))
            $stop;
        end
...
module t_rand_basic_types_constrained;
    ...
    initial begin
        ...
        repeat(10) begin
            ...
            `DBG(("enum_value: %0d, random_32_bit: %h, single_bit: %b", 
