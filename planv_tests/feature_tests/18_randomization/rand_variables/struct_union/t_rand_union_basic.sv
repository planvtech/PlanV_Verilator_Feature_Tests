...
//
// This test validates: constraints on union types
...
class PackedUnionTest;
    ...
    function void check();
        if (!(my_union.int_value inside {[0:100]})) begin
            `DBG(("Error: my_union.int_value = %0d is out of bounds", my_union.int_value))
            $stop;
        end
        `DBG(("PackedUnion constraints validated successfully."))
    endfunction
endclass

module t_rand_union_basic;
    ...
    initial begin
        ...
        repeat(10) begin
            ...
            `DBG(("int_value: %0d, bits: %h", union_test.my_union.int_value, union_test.my_union.bits))
        end
        `TEST_PASS
    end
endmodule
