...
//
// This test validates: complex struct constraints with nested members
...
class ArrayStruct;
    ...
    function void print();
        foreach (s1.arr[i]) begin `DBG(("s1.arr[%0d] = %0d", i, s1.arr[i])) end
        foreach (s1.arr_3[i]) begin `DBG(("s1.arr_3[%0d] = %0d", i, s1.arr_3[i])) end
        foreach (s1.arr_4[i]) begin `DBG(("s1.arr_4[%0d] = %0d", i, s1.arr_4[i])) end
        foreach (s2.arr[i]) begin `DBG(("s2.arr[%0d] = %0d", i, s2.arr[i])) end
        foreach (s3.arr[i]) begin `DBG(("s3.arr[%0d] = %0d", i, s3.arr[i])) end
        foreach (s4.arr[i]) begin `DBG(("s4.arr[\"%s\"] = %0d", i, s4.arr[i])) end
        foreach (s5.arr[i, j]) begin `DBG(("s5.arr[%0d][%0d] = %0d", i, j, s5.arr[i][j])) end
        foreach (s6.mix_arr[i, j]) begin `DBG(("s6.mix_arr[%0d][%0d] = %0d", i, j, s6.mix_arr[i][j])) end
    endfunction
...
class StructArray;
    ...
    function void print();
        foreach (s_arr[i]) begin
            foreach (s_arr[i].arr[j]) begin
                `DBG(("s_arr[%0d].arr[%0d] = %0d", i, j, s_arr[i].arr[j]))
            end
            `DBG(("s_arr[%0d].a = %0d", i, s_arr[i].a))
            `DBG(("s_arr[%0d].b = %0d", i, s_arr[i].b))
            `DBG(("s_arr[%0d].c = %0d", i, s_arr[i].c))
        end
        foreach (s_2d_arr[i, j]) begin
            foreach (s_2d_arr[i][j].arr[k]) begin
                `DBG(("s_2d_arr[%0d][%0d].arr[%0d] = %0d", i, j, k, s_2d_arr[i][j].arr[k]))
            end
            `DBG(("s_2d_arr[%0d][%0d].a = %0d", i, j, s_2d_arr[i][j].a))
            `DBG(("s_2d_arr[%0d][%0d].b = %0d", i, j, s_2d_arr[i][j].b))
            `DBG(("s_2d_arr[%0d][%0d].c = %0d", i, j, s_2d_arr[i][j].c))
        end
        foreach (s_dyn_arr[i]) begin
            foreach (s_dyn_arr[i].arr[j]) begin
                `DBG(("s_dyn_arr[%0d].arr[%0d] = %0d", i, j, s_dyn_arr[i].arr[j]))
            end
            `DBG(("s_dyn_arr[%0d].a = %0d", i, s_dyn_arr[i].a))
            `DBG(("s_dyn_arr[%0d].b = %0d", i, s_dyn_arr[i].b))
            `DBG(("s_dyn_arr[%0d].c = %0d", i, s_dyn_arr[i].c))
        end
        foreach (s_que_arr[i]) begin
            foreach (s_que_arr[i].arr[j]) begin
                `DBG(("s_que_arr[%0d].arr[%0d] = %0d", i, j, s_que_arr[i].arr[j]))
            end
            `DBG(("s_que_arr[%0d].a = %0d", i, s_que_arr[i].a))
            `DBG(("s_que_arr[%0d].b = %0d", i, s_que_arr[i].b))
            `DBG(("s_que_arr[%0d].c = %0d", i, s_que_arr[i].c))
        end
        foreach (s_assoc_arr["x"].arr[j]) begin
            `DBG(("s_assoc_arr[x].arr[%0d] = %0d", j, s_assoc_arr["x"].arr[j]))
        end
        `DBG(("s_assoc_arr[x].a = %0d", s_assoc_arr["x"].a))
        `DBG(("s_assoc_arr[x].b = %0d", s_assoc_arr["x"].b))
        `DBG(("s_assoc_arr[x].c = %0d", s_assoc_arr["x"].c))
        foreach (s_assoc_arr["y"].arr[j]) begin
            `DBG(("s_assoc_arr[y].arr[%0d] = %0d", j, s_assoc_arr["y"].arr[j]))
        end
        `DBG(("s_assoc_arr[y].a = %0d", s_assoc_arr["y"].a))
        `DBG(("s_assoc_arr[y].b = %0d", s_assoc_arr["y"].b))
        `DBG(("s_assoc_arr[y].c = %0d", s_assoc_arr["y"].c))
        foreach (s_assoc_arr["long_string_index"].arr[j]) begin
            `DBG(("s_assoc_arr[long_string_index].arr[%0d] = %0d", j, s_assoc_arr["long_string_index"].arr[j]))
        end
        `DBG(("s_assoc_arr[long_string_index].a = %0d", s_assoc_arr["long_string_index"].a))
        `DBG(("s_assoc_arr[long_string_index].b = %0d", s_assoc_arr["long_string_index"].b))
        `DBG(("s_assoc_arr[long_string_index].c = %0d", s_assoc_arr["long_string_index"].c))
    endfunction
...
class MixedStructure;
    ...
    function void print();
        foreach (s_arr[i]) begin
            ...
            foreach (s_arr[i].dyn[j]) begin
                `DBG(("s_arr[%0d].dyn[%0d] = %0d", i, j, s_arr[i].dyn[j]))
            end
            foreach (s_arr[i].que[j]) begin
                `DBG(("s_arr[%0d].que[%0d] = %0d", i, j, s_arr[i].que[j]))
            end
            `DBG(("s_arr[%0d].assoc[\"x\"] = %0d", i, s_arr[i].assoc["x"]))
            `DBG(("s_arr[%0d].assoc[\"y\"] = %0d", i, s_arr[i].assoc["y"]))
            `DBG(("s_arr[%0d].a = %0d", i, s_arr[i].a))
            `DBG(("s_arr[%0d].b = %0d", i, s_arr[i].b))
            `DBG(("s_arr[%0d].c = %0d", i, s_arr[i].c))
        end
...
module t_rand_struct_complex;
    ...
    initial begin
        ...
        as_c.self_test();
        as_c.print();
        `DBG((" ArrayStruct passed! \n"))

        success = sa_c.randomize();
        if (success != 1) $stop;
        sa_c.self_test();
        sa_c.print();
        `DBG((" StructArray passed! \n"))

        success = mixed_c.randomize();
        if (success != 1) $stop;
        mixed_c.self_test();
        mixed_c.print();
        `DBG((" MixedStructure passed! \n"))
