...
//
// This test validates: basic struct constraints
...
class PackedStructTest;
    ...
    function void check();
        if (my_struct.byte_value != 8'hA0) begin
            `DBG(("Error: my_struct.byte_value = %h is not equal to 8'hA0", my_struct.byte_value))
            $stop;
        end
        if (!(my_struct.int_value inside {[0:100]})) begin
            `DBG(("Error: my_struct.int_value = %d is out of bounds", my_struct.int_value))
            $stop;
        end
        `DBG(("PackedStruct constraints validated successfully."))
    endfunction
endclass
...
class UnpackedStructTest;
    ...
    function void check();
        if (!(my_unpacked_struct.byte_value inside {8'hA0, 8'hB0, 8'hC0})) begin
            `DBG(("Error: my_unpacked_struct.byte_value = %h is out of bounds", my_unpacked_struct.byte_value))
            $stop;
        end
        if (!(my_unpacked_struct.int_value inside {[50:150]})) begin
            `DBG(("Error: my_unpacked_struct.int_value = %d is out of bounds", my_unpacked_struct.int_value))
            $stop;
        end
        `DBG(("UnpackedStruct constraints validated successfully."))
    endfunction
endclass

module t_rand_struct_basic;
    ...
    initial begin
        ...
        repeat(10) begin
            ...
            `DBG(("PackedStruct: byte_value: %h, int_value: %0d", packed_struct_test.my_struct.byte_value, packed_struct_test.my_struct.int_value))
        end
        ...
        repeat(10) begin
            ...
            `DBG(("UnpackedStruct: byte_value: %h, int_value: %0d", unpacked_struct_test.my_unpacked_struct.byte_value, unpacked_struct_test.my_unpacked_struct.int_value))
        end
