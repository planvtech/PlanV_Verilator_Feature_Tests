...
//
// This test validates: constraints on struct and union types
...
class StructUnionTest;
    ...
    function void check();
        if (!(my_struct.a inside {[0:15]})) begin
            `DBG(("Error: my_struct.a = %b is out of bounds", my_struct.a))
            $stop;
        end
        if (!(my_struct.b inside {[0:255]})) begin
            `DBG(("Error: my_struct.b = %b is out of bounds", my_struct.b))
            $stop;
        end
        if (!(my_union.raw_bits inside {[0:4095]})) begin
            `DBG(("Error: my_union.raw_bits = %b is out of bounds", my_union.raw_bits))
            $stop;
        end
        if (!(my_inner_union.raw_bits inside {[0:65535]})) begin
            `DBG(("Error: my_inner_union.raw_bits = %b is out of bounds", my_inner_union.raw_bits))
            $stop;
        end
        if (!(my_outer_union.raw_data inside {[0:65535]})) begin
            `DBG(("Error: my_outer_union.raw_data = %b is out of bounds", my_outer_union.raw_data))
            $stop;
        end
...
module t_rand_struct_union_basic;
    ...
    initial begin
        ...
        repeat(10) begin
            ...
            `DBG(("PackedStruct: a: %b, b: %b", test.my_struct.a, test.my_struct.b))
            `DBG(("Union: raw_bits: %b, s: %b", test.my_union.raw_bits, test.my_union.s))
            `DBG(("Inner Union: a: %b, b: %b, raw_bits: %b", test.my_inner_union.u.s.a, test.my_inner_union.u.s.b, test.my_inner_union.raw_bits))
            `DBG(("Outer Union: raw_data: %b", test.my_outer_union.raw_data))
            `DBG(("***************************"))
        end
        `TEST_PASS
    end
